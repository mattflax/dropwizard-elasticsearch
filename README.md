...
* `servers`: A list of servers for usage with the created TransportClient
...
An example configuration file for creating a client could look like this:
...
The order of precedence is: `servers`/`clusterName` > `settings` > `settingsFile`, meaning that
...
Elasticsearch 5 does not allow the creation of a NodeClient, and it is removed from this version of
the connector.
